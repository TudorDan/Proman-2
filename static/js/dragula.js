...
function dragula (initialContainers, options) {
  ...
  async function release (e) {
    ...
    if (dropTarget && ((_copy && o.copySortSource) || (!_copy || dropTarget !== _source))) {
      ...
      const tasks = dropTarget.querySelectorAll("[id^='taskk-']");
      let taskRank = 0;
      for (let task of tasks) {
        const taskId = parseInt(task.id.replace('taskk-', ''));
        const statusId = parseInt(task.parentElement.id.replace('statuss-', ''));
        taskRank += 100;
        let dataToBePosted = {
          id: taskId,
          status_id: statusId,
